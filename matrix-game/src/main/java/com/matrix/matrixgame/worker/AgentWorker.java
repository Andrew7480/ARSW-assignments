...
public class AgentWorker extends Thread {
    ...
    public AgentWorker(Agent agent, Board board, GameEngine engine, CyclicBarrier barrier) {
        this.agent = agent;
        this.board = board;
        this.engine = engine;
    ...
    public void run() {
        try {
            while (engine.isRunning()) {
                Position next = agent.calculateNextMove(board);
                agent.setNextPosition(next);
